...
@contextmanager
def day_block(is_today=False):
    """Prints a day block in between two separator lines

    Keyword Arguments:
        is_today {bool} -- Whether to make the block bold or not (default: {False})
    """
    style = Style.BRIGHT if is_today else ''
    print(style + "="*40)
    try:
        yield
    finally:
